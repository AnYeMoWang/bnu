...
def ke_upload_view(request):
    filetype = request.GET.get('dir')
    ext_allowed = ['gif', 'jpg', 'jpeg', 'png']
    max_size = 26214400
    today = datetime.datetime.today()
    save_dir = 'upload/%s/%d/%d/%d/' % (filetype, today.year, today.month, today.day)
    ...
    if request.method == 'POST':
        ...
        if filetype == 'image':
            if ext.lower() not in ext_allowed:
                return HttpResponse(json.dumps(
        ...
        if file.size > max_size:
            return HttpResponse(json.dumps(
                {'error': 1, 'message': u'上传的文件大小不能超过25MB'}
            ))
...
def ke_man_view(request):
    ...
    if os.listdir(current_path):
        for file_name in os.listdir(current_path):
            ...
            dicts["datetime"] = datetime.datetime.now().strftime(
                '%Y-%m-%d %H:%M:%S')
